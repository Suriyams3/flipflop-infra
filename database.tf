...
resource "aws_instance" "db_server" {
  ...
  user_data = <<-EOF
              ...
              # 5. Connect and transition local root user securely using temporary instance token
              mysql --connect-expired-password -u root -p"$TEMP_ROOT_PASS" --execute="ALTER USER 'root'@'localhost' IDENTIFIED BY '$FETCHED_ROOT_PASS';"

              # 5a. Properly create the remote root user using shell interpolation (avoids SQL file variable trap)
              mysql -u root -p"$FETCHED_ROOT_PASS" --execute="CREATE USER IF NOT EXISTS 'root'@'%' IDENTIFIED BY '$FETCHED_ROOT_PASS';"
              mysql -u root -p"$FETCHED_ROOT_PASS" --execute="GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' WITH GRANT OPTION;"
              mysql -u root -p"$FETCHED_ROOT_PASS" --execute="FLUSH PRIVILEGES;"

              # 5b. Build out pure database structure scripts without variable dependencies
              ...
              INSERT INTO credit_card_details (account_number, credit_card_number, expiry_date, credit_points_available, credit_points_expiry_date, cibil_score) VALUES
              ...
              INSERT INTO profile_details (account_number, first_name, last_name, age, sex, marital_status, address, phone_number) VALUES
              ...
              INSERT INTO account_details (account_number, account_balance, account_open_date, debit_card_number, debit_card_expiry_date) VALUES
              ...
              INSERT INTO credit_card_offers (offer_name, sponsor_name, offer_valid_upto, discount_percentage, offer_description, reward_points_required, cibil_score_required) VALUES
              ...
              # 6. Apply schemas safely using clean credentials
